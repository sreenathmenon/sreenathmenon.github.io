...
class Ce_img_aws_ext
{
	...
	public function update_valid_params()
	{
		...
		//aws_auto_url
		if (isset($this->EE->config->_global_vars['ce_image_aws_auto_url']) && $this->EE->config->_global_vars['ce_image_aws_auto_url'] != '') //global array
		{
			self::$aws_auto_url = $this->EE->config->_global_vars['ce_image_aws_auto_url'];
		}
		else if ($this->EE->config->item('ce_image_aws_auto_url') != FALSE) //config
		{
			self::$aws_auto_url = $this->EE->config->item('ce_image_aws_auto_url');
		}
		...
		//aws_cloudfront
		self::$aws_cloudfront_url = $this->EE->config->item('ce_image_aws_cloudfront_url');
	}
	...
	public function pre_parse( $tagdata, $variables, $var_prefix )
	{
		...
		$aws_auto_url = $this->EE->TMPL->fetch_param('aws_auto_url', self::$aws_auto_url );
		...
		$aws_cloudfront_url = $this->EE->TMPL->fetch_param( 'aws_cloudfront_url', self::$aws_cloudfront_url );
