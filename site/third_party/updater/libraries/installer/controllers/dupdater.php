...
*/
        ...
        if (file_exists(EE_APPPATH.'libraries/Smartforge.php') === true) {
            $this->load->library('logger'); // Logger MUST be before add_package_path
        ...
        // EE 2.8+
        if (version_compare($this->version, '280', '>=')) {
            if (!defined('PATH_THEMES')) {
                define('PATH_THEMES', $this->server['themes']);
            }
        }
    ...
    public function copy_files_prepare()
    {
        ...
        */
        ...
        // EE 2.1.0 > 2.2.0 doesn't like the above!
        $out['dirs'][] = 'system/codeigniter';
    ...
    public function update_ee()
    {
        ...
        $this->load->add_package_path($installer_dir);

        if (file_exists($installer_dir.'libraries/Progress.php')) {
            $this->load->library('progress');
        }

        if (file_exists($installer_dir.'libraries/Layout.php')) {
        ...
        $this->config->_update_config(array('app_version' => $this->version) );
