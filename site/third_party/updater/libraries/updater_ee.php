...
class Updater_ee
{
    ...
    public function copyInstaller($action_obj)
    {
        ...
        try
        {
            ...
            foreach (scandir($root.'system/installer/libraries/') as $file) {
                if ($file == '.' || $file == '..') continue;

                $this->EE->updater_transfer->upload('system', $root.'system/installer/libraries/'.$file, 'installer/libraries/'.$file, 'file', true);
            }

            if (version_compare($this->EE->config->item('app_version'), '220', '>=')) {
                foreach (scandir($root.'system/installer/core/') as $file) {
                    if ($file == '.' || $file == '..') continue;

                    $this->EE->updater_transfer->upload('system', $root.'system/installer/core/'.$file, 'installer/core/'.$file, 'file', true);
                }
            }
            ...
            // Special Extensions Library.. (Since: EE 2.7.0)
            if (file_exists($root.'system/installer/libraries/Extensions.php')) {
                $this->EE->updater_transfer->upload('system', $root.'system/installer/libraries/Extensions.php', 'installer/libraries/Extensions.php', 'file', TRUE);
            }
