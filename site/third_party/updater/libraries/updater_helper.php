...
class Updater_helper
{
    ...
    {
        if (class_exists('ZipArchive') === true && $this->EE->updater->settings['use_zip_ext'] == 'yes') {
            $zip = new ZipArchive();
        ...
        if ($type == 'gjs') {
            if ( isset($this->EE->session->cache['inline_js'][$this->package_name]) == false ) {
                ...
                $js = " var Updater = Updater ? Updater : {};
                        Updater.XID = '{$XID}';
                        Updater.ACT_ID = '{$ACT_id}';
                        Updater.ACT_URL = '{$ACT_url}';
                        Updater.MCP_AJAX_URL = '{$MCP_AJAX_url}';
                        Updater.SITE_URL = '{$site_url}';
                        Updater.THEME_URL = '{$theme_url}';
                        Updater.AUTH_KEY = '{$this->EE->updater->settings['auth_key']}';
                ";
