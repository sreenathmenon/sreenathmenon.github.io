...
class Updater
{
    ...
    public function actionGeneralRouter($mcp_task=false)
    {
        ...
        // -----------------------------------------
        if (version_compare(APP_VER, '2.8.0', '<')  && method_exists($this->EE->security, 'generate_xid')) {
            //$this->EE->security->restore_xid($this->EE->input->post('XID'));
            $XID = $this->EE->security->generate_xid();
            @header('X-Updater-XID: ' . $XID);
        }

        if (method_exists($this->EE->security, 'restore_xid')) {
            $this->EE->security->restore_xid($this->EE->input->post('XID'));
        }
        ...
        // Task
        $task = $this->EE->input->get_post('task');

        // Auth Key
        $authenticated = false;
        $auth_key = $this->EE->input->get_post('auth_key');

        if ($auth_key != false) {
            if ($this->EE->updater->settings['auth_key'] == $auth_key) {
                $authenticated = true;
            }
        } else {
        ...
        if (!$authenticated) {
            header('X-Updater-Error: No Super Admin Or missing/wrong Auth Key');
            header('X-Updater-Group: '. $this->EE->session->userdata['group_id']);
            header('X-Updater-Member: '. $this->EE->session->userdata['member_id']);
            header('X-Updater-AuthKey: '. $auth_key);
        ...
        if (method_exists('Updater', $task) == false) {
            header('X-Updater-Error: No Task');
            header("HTTP/1.0 403 Forbidden");
            exit();
        }
