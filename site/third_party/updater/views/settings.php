	...
	<tbody>
		<tr>
			<td colspan="2" class="subtable">
				<table>
					<tr>
						<td style="width:250px"><label>Authentication Key</label></td>
						<td>
							<input name="settings[auth_key]" value="<?=$settings['auth_key']?>" type="text" <?php if (isset($override_settings['auth_key'])):?>disabled="disabled"<?php endif;?> >
							<small>When set, Updater will use this key to authenticate AJAX requests. Recommended if you are having cookie/session/cross-domain issues.</small>
						</td>
					</tr>
				</table>
			</td>
