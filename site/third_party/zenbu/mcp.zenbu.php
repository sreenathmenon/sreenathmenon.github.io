...
class Zenbu_mcp {
	...
	{
		//	----------------------------------------
		//	Restore XID to avoid invalid action errors
		//	----------------------------------------
		...
		*/
		if($this->EE->input->get_post('rule_id', TRUE))
		{
			...
			$rule_data = $this->EE->zenbu_get->_get_cached_filters();

			$other_rules = array("limit" => "limit", "orderby" => "default_order", "sort" => "default_sort", "perpage" => "perpage");

			if(isset($rule_data['rule']))
			{
				$rules = $rule_data['rule'];
			...
			foreach($other_rules as $elem => $var)
			{
				if(isset($rule_data[$elem]))
				{
					// This will be passed in entry_get function
					$settings['setting']['general'][$var] = $rule_data[$elem];
					// This is used to refresh the default values above
					$$var = $rule_data[$elem];					
				}
			}
		...
		*/
		...
		foreach($vars_order['field_order'] as $key => $col_name)
		{
			$new_col_name = str_replace('show_', '', $col_name);
			$new_col_name = $new_col_name == 'view' ? 'live_look' : $new_col_name;
			$new_col_name = $new_col_name == 'sticky' ? 'is_sticky' : $new_col_name;
			$new_col_name = $new_col_name == 'id' && $new_col_name != $vars_rules['orderby_val'] ? '#' : $new_col_name;
			$new_col_name = $new_col_name == 'categories' ? 'category' : $new_col_name;
			$new_col_name = strncmp($new_col_name, 'field_', 6) == 0 && isset($vars_fields['field'][str_replace('field_', '', $new_col_name)]) ? $vars_fields['field'][str_replace('field_', '', $new_col_name)] : $new_col_name;
			$column_headers['sort'][$col_name] = $new_col_name;
			$column_headers[$col_name] = $this->EE->lang->line($new_col_name);
		}

		/**
		*	===========================================
		*	Extension Hook zenbu_modify_column_headers
		*	===========================================
		*
		*	Modifies the labels for each visible column in Zenbu.
		*	@param  $column_headers 	array 		The column header titles 
		*	@return $column_headers 	array		The modified column header array
		*/
		...
		$vars_order['column_headers'] = $column_headers;
	...
	*/
	function multi_edit()
	{
		...
		case "add_categories" OR "remove_categories":
			...
			$add_or_remove = $this->EE->input->get_post('action', TRUE) == 'remove_categories' ? 'remove' : 'add';

			if(version_compare(APP_VER, '2.6', '>'))
			{
				$this->EE->view->cp_page_title = $this->EE->lang->line('multi_entry_category_editor') . ' - ' . strtoupper($this->EE->lang->line($add_or_remove)) . ' ' . $this->EE->lang->line('categories');
			} else {
				$this->EE->cp->set_variable('cp_page_title', $this->EE->lang->line('multi_entry_category_editor') . ' - ' . strtoupper($this->EE->lang->line($add_or_remove)) . ' ' . $this->EE->lang->line('categories'));
			}
			
			$category_data = $this->EE->zenbu_get->_get_category_dropdowns();
			
			//	----------------------------------------
			//	Get entry data based on submitted array, and
			//	associate entries to categories
			//	----------------------------------------

			$entry_data = $this->EE->zenbu_get->_get_core_entry_data($entry_ids);

			//	----------------------------------------
			//	Get a quick list of all selected entry titles
			//	----------------------------------------

			$vars['entry_title_list'] = '';

			foreach($entry_data as $entry_id => $e_data)
			{
				$vars['entry_title_list'] .= $e_data['title'] . ', ';
			}

			$vars['entry_title_list'] = rtrim($vars['entry_title_list'], ', ');

			$entry_categories = $this->EE->zenbu_get->_get_category_list($entry_ids);

			$entry_titles_per_category = array();

			if( ! empty($entry_categories) )
			{
				foreach($entry_categories as $entry_id => $cat_data)
				{
					foreach($cat_data as $cat_id => $cat_name)
					{
						if( ! isset($entry_titles_per_category[$cat_id]) )
						{
							$entry_titles_per_category[$cat_id] = '';
						}

						$entry_titles_per_category[$cat_id] .= $entry_data[$entry_id]['title'] . ', ';
					}
				}
			}

			foreach($entry_titles_per_category as $cat_id => $str)
			{
				$vars['entry_titles_per_category'][$cat_id] = trim($str, ', ');
			}

			//	----------------------------------------
			//	Get channel_ids of selected entries
			//	----------------------------------------

			foreach($entry_data as $entry_id => $data)
			{
				$channels[$data['channel_id']] = $data['channel_id'];
			}

			//	----------------------------------------
			//	Check for the presence of *any* common 
			//	channel category groups (for multi-channel entries).
			//	Throw an error is this isn't the case
			//	----------------------------------------
			
			$channel_data = $this->EE->zenbu_get->_get_channel_data($this->member_group_id);
			$channel_data = $channel_data['channels']['channel_data'];
			
			$category_groups = array();

			foreach($channels as $channel_id)
			{
				$category_groups[$channel_id] = $channel_data[$channel_id]['channel_category_groups'];
			}

			$common_category_groups = (count($category_groups) > 1 && ! empty($category_groups) ) ? call_user_func_array('array_intersect', $category_groups) : 1;

			//var_dump($common_category_groups);

			if( empty($common_category_groups) )
			{
				show_error(lang('no_category_group_match'));
			}

			//	----------------------------------------
			//	Use those entry channel_ids to find all
			//	*common* category groups. Based on category_group_id.
			//	----------------------------------------
			...
			foreach($channels as $channel_id)
			{
				if( isset($category_data['categories']['ch_id_'.$channel_id]['category_structure']) )
				{
					$category_structure = &$category_data['categories']['ch_id_'.$channel_id]['category_structure'];

					foreach($category_structure as $cat_group_id => $data)
					{
						//	----------------------------------------
						//	Build category group array of only common
						//	category groups across all entries. 
						//	(eg. Channel A has cat groups 1, 2, and 
						//	Channel B has cat group 2, only show 2.)
						//	----------------------------------------
						if( ! empty($common_category_groups) && is_array($common_category_groups))
						{
							foreach($common_category_groups as $common_cat_group_id)
							{
								$vars['categories'][$common_cat_group_id] = $category_structure[$common_cat_group_id];
							}
						} else {
							$vars['categories'][$cat_group_id] = $data;
						}
					}
				}
			}
			...
			//	----------------------------------------
			//	Build a quick category group name array for the view
			//	----------------------------------------

			$vars['category_group_names'] = $category_data['category_groups'];

			$vars['type'] = $add_or_remove;

			$hidden['type'] = $add_or_remove;

			$hidden['entry_ids'] = implode($entry_ids, '|');
			
			$vars_hidden['hidden_fields'] = form_hidden($hidden);
			
			$vars_action['action_url'] = "C=content_edit".AMP."M=multi_entry_category_update".AMP."from_zenbu=y";
			
			$vars = array_merge($vars, $vars_hidden, $vars_action);
	...
	function manage_searches()
	{
		...
		*/
		...
		$vars['edit_search_action_url'] = "C=addons_modules".AMP."M=show_module_cp".AMP."module=".$this->addon_short_name.AMP."method=update_search";
	...
	function settings()
	{
		...
		*/
		...
		$labels_std = array(
			...
			"show_pages"			=> $this->EE->lang->line('page_uri'),
		);
		...
		*/
		...
		$default_start_filters_array = array(
			...
			'pages'				=> $this->EE->lang->line('page_uri'),
