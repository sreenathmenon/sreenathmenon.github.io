...
class Zenbu_get extends Zenbu_mcp {
	...
	*/
	function _get_first_and_orderby_dropdown($settings)
	{
		...
		$output['rules_first_dropdown']['ch_id_0']['dropdown_labels']['pages']											= $this->EE->lang->line('page_uri');
		...
		if($results->num_rows() > 0)
		{
			...
			foreach($results->result_array() as $row)
			{
				...
				$output['rules_first_dropdown']['ch_id_'.$ch_id]['dropdown_labels']['pages']											= $this->EE->lang->line('page_uri');
	...
	*/
	...
	function get_assigned_data()
	{
		...
		{
			$sql = "/* Zenbu get_assigned_data - channels */ SELECT channel_id, channel_title, cat_group FROM exp_channels WHERE site_id = " . $this->site_id . " ORDER BY channel_title ASC";
		} else {
			$sql = "/* Zenbu get_assigned_data - channels */ SELECT cmg.channel_id, c.channel_title, c.cat_group 
					FROM exp_channels c 
		...
		if($results->num_rows() > 0)
		{
			foreach($results->result_array() as $row)
			{
				$output['channel_ids'][] = $row['channel_id'];
				$output['channels'][$row['channel_id']]['channel_id'] 			= $row['channel_id'];
				$output['channels'][$row['channel_id']]['channel_title'] 		= $row['channel_title'];
				$output['channels'][$row['channel_id']]['channel_category_groups'] 	= explode('|', trim($row['cat_group'], '|'));
			}
		}
	...
	*/
	function _get_search_rules($rule_id)
	{
		...
		return $output;
	} // END _get_search_rules()

	// --------------------------------------------------------------------


	/**
	* function _get_cached_filters
	* Retrieve temporary filter rule data. 
	* Used when user returns to Zenbu with pre-filtered rules
	* @return	array The filter data, including rules, limit, etc.
	*/
	function _get_cached_filters()
	{

		// Return data if already cached
		if($this->EE->session->cache('zenbu', 'cached_filters'))
		{
			return $this->EE->session->cache('zenbu', 'cached_filters');
		}

		$results = $this->EE->db->query("/* Zenbu _get_cached_filters */ \n SELECT * 
		FROM exp_zenbu_filter_cache WHERE member_id = " . $this->member_id . " AND site_id = " . $this->site_id . " ORDER BY save_date DESC LIMIT 1");
		...
		if($results->num_rows() > 0)
		{
			foreach($results->result_array() as $row)
			{
				$output = unserialize($row['filter_rules']);
			}
		}
		...
		$this->EE->zenbu_db->clean_up_filter_cache();
		
		$this->EE->session->set_cache('zenbu', 'cached_filters', $output);

		return $output;
	} // END _get_cached_filters()
	...
	function _get_category_dropdowns($installed = array())
	{
		...
		$cat_array = array();
		$category_permissions_installed = ( ! empty($installed) && in_array('Category_permissions_ext', $installed['extensions'])) ? TRUE : FALSE;
		...
		$results = $this->EE->db->query("/* Zenbu category query */\nSELECT REPLACE(cat_group, '|', ',') as cat_groups, channel_id, channel_title FROM exp_channels WHERE site_id = ".$this->site_id);
		if($results->num_rows() > 0)
		{
			foreach($results->result_array() as $row)
			{
				...
				//	----------------------------------------
				$this->output['categories']['ch_id_'.$ch_id]['channel_title']           = $row['channel_title'];
				...
				// Need to remove the first and last comma if "None" was also selected among category groups in Channel preferences
				// Personally, "None" shouldn't even be an option in the multiselect field!
				$cat_groups = trim($cat_groups, ',');
					...
					*/
					...
					foreach($cat_array as $cat_id => $array)
					{
						...
						if( $array['parent_id'] == 0 )
						{
							...
							$this->output['categories']['ch_id_0']['category_structure'][$array['group_id']][$cat_id] = $array['cat_name'];

							// With channel
							$this->output['categories']['ch_id_'.$ch_id]['dropdown_labels'][$array['group_name']][$cat_id]  = $array['cat_name'];
							$this->output['categories']['ch_id_'.$ch_id]['category_structure'][$array['group_id']][$cat_id] = $array['cat_name'];
							...
							// Build a quick category_group_id : category_group_name array
							$this->output['category_groups'][$array['group_id']] = $array['group_name'];
	...
	function _get_sub_categories($parent_id, $cat_array, $level, $channel_id, $permitted_cat_array = array())
	{
		...
		foreach($cat_array as $cat_id => $array)
		{
			if($parent_id == $array['parent_id'])
			{
				...
				$sub_indicator = str_repeat(NBS.NBS.NBS.NBS.NBS.NBS, $level).'&#8970;';
				...
				// No channel
				$this->output['categories']['ch_id_0']['dropdown_labels'][$cat_array[$cat_id]['group_name']][$cat_array[$cat_id]['id']] = $sub_indicator . NBS . $cat_array[$cat_id]['cat_name'];
				$this->output['categories']['ch_id_0']['category_structure'][$cat_array[$cat_id]['group_id']][$cat_array[$cat_id]['id']] = $sub_indicator . NBS . $cat_array[$cat_id]['cat_name'];

				// With channel
				$this->output['categories']['ch_id_'.$channel_id]['dropdown_labels'][$cat_array[$cat_id]['group_name']][$cat_array[$cat_id]['id']] = $sub_indicator . NBS . $cat_array[$cat_id]['cat_name'];
				$this->output['categories']['ch_id_'.$channel_id]['category_structure'][$cat_array[$cat_id]['group_id']][$cat_array[$cat_id]['id']] = $sub_indicator . NBS . $cat_array[$cat_id]['cat_name'];
	...
	function _get_channel_data($member_group_id)
	{
		...
		foreach($assigned_data['channels'] as $channel_id => $channel_data)
		{
			$output['channels']['dropdown_labels'][$channel_id]                    = $channel_data['channel_title'];
			$output['channels']['channel_data'][$channel_id]['id']                 = $channel_data['channel_id'];;
			$output['channels']['channel_data'][$channel_id]['channel_title']      = $channel_data['channel_title'];
			$output['channels']['channel_data'][$channel_id]['channel_category_groups'] = $channel_data['channel_category_groups'];
		}
	...
	*/
	function _get_installed_addons()
	{
		...
		//	----------------------------------------
		//	Separating Modules and Extension queries
		//	just in case combining them is just giving
		//	too much of a large result. 
		//	Eg. large exp_extensions table
		//	----------------------------------------
		...
		$query = $this->EE->db->query("/* Zenbu _get_installed_addons */ \n SELECT e.extension_id, e.class, e.version 
		FROM exp_extensions e");
	...
	{	
		...
		*/
		...
		if( ! empty($channel_id) && count($channel_id) == 1 )
		{
			...
			$this->EE->db->where_in("channel_titles.channel_id", $assigned_data['channel_ids']);
		}
		...
		switch ($orderby)
		{
			...
			case "pages":
				...
				$this->EE->db->_protect_identifiers = FALSE;
				$entry_fixed_order = implode(',', array_keys($output_pages));
				$this->EE->db->order_by('FIELD(' . $this->EE->db->dbprefix . 'channel_titles.entry_id, '.$entry_fixed_order.')', '');
				$this->EE->db->_protect_identifiers = TRUE;
			break;
			case ( ! empty($orderby)):
		...
		*/
		...
		$output['compiled_query'] = $this->EE->db->last_query(); //echo $output['compiled_query'];
		...
		*/
		if($final_results->num_rows() > 0)
		{
				...
				foreach($final_results->result_array() as $row)
				{
					...
					$filter_array	= array("return_to_zenbu" => "y", "perpage" => $perpage);
					...
					*/
					...
					// Pages
					$output['entry'][$row['entry_id']]['pages'] = isset($output_pages[$row['entry_id']]) ? $output_pages[$row['entry_id']] : NBS;
