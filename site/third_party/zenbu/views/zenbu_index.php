	...
	<div id="filterMenu">
		...
		<div id="rulefields">
			<?php if(isset($rules) && ! empty($rules)) :?>
				...
				<?=form_fieldset()?>
					<div class="limit">
						<?=form_dropdown('limit', $limit['dropdown_labels'], ($this->input->get_post('limit') == "") ? $limit_val : (int) $this->input->get_post('limit'), 'class=""')?>
						&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
						<?=lang('orderby')?>&nbsp;
						<span class="orderby">
			...
			*/
			...
			<?=form_fieldset()?>
				<div class="limit">
					<?=form_dropdown('limit', $limit['dropdown_labels'], ($this->input->get_post('limit') == "") ? 25 : (int) $this->input->get_post('limit'), 'class=""')?>
					&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
					<?=lang('orderby')?>&nbsp;
					<span class="orderby">
		...
		<?php foreach($field_order as $order => $col_name) :?>
			...
			<?php if(substr($col_name, 0, 6) != 'field_') :?>
				...
				<?php if(isset(${$col_name}) && ${$col_name} == "y") :?>
					...
					<th id="<?=$column_headers['sort'][$col_name]?>" class="<?=($orderby_val == $column_headers['sort'][$col_name] && $sort_val == "desc") ? 'headerSortUp' : '' ;?><?=($orderby_val == $column_headers['sort'][$col_name] && $sort_val == "asc") ? 'headerSortDown' : '' ;?>"><?=$column_headers[$col_name]?></th>
			...
			<?php else :?>
				...
				<?php if(isset($field[$field_id])):?>
					...
					<th id="field_id_<?=$field_id?>"><?=$column_headers[$col_name]?></th>
