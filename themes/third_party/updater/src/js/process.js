;(function(global, $){
    ...
    function startBackupFiles(id) {
        ...
        $.ajax({url: Updater.ACT_URL+'&task=backup_files_prepare' + '&cache=' + Updater.generateRandomString(),
            dataType: 'json', type: 'POST', data: {
                XID: Updater.getXID(),
                auth_key: Updater.AUTH_KEY,
                time: backupDate
            },
            error: function(xhr){
                Updater.setXID(xhr);
    ...
    function backupFilesDirs(index) {
        ...
        $.ajax({url: Updater.ACT_URL+'&task=backup_files' + '&cache=' + Updater.generateRandomString(),
    ...
    function startBackupDb(id) {
        ...
        $.ajax({url: Updater.ACT_URL+'&task=backup_database_prepare' + '&cache=' + Updater.generateRandomString(),
            dataType: 'json', type: 'POST', data: {XID: Updater.getXID(), auth_key: Updater.AUTH_KEY},
            error: function(xhr){
                Updater.setXID(xhr);
                triggerError(xhr);
            },
    ...
    function backupDbTables(index) {
        ...
        $.ajax({url: Updater.ACT_URL+'&task=backup_database' + '&cache=' + Updater.generateRandomString(),
            dataType: 'json', type: 'POST',
            data: {
                action: 'backup',
    ...
    function startGeneralAction(id) {
        ...
        $.ajax({url: Updater.ACT_URL+'&task=addon_move_files' + '&cache=' + Updater.generateRandomString(),
            dataType: 'json', type: 'POST',
            data: {
            ...
            success: function(rData, textStatus, xhr){
                ...
                */
                ...
                if (currentAction.type == 'addon' || currentAction.type == 'ee_forum') {
                    // Give EE 2.7+ time to redo XID! sigh
                    setTimeout(function(){
                        endGeneralAction(id);
                    }, 500);
                    return;
    ...
    function eeUpdateInit(id) {
        ...
        $.ajax({url: Updater.ACT_URL+'&task=ee_update_init' + '&cache=' + Updater.generateRandomString(),
            dataType: 'json', type: 'POST',
            data: {
                action: 'site_offline',
